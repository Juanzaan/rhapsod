...
| `RHAPSOD_SPOTIFY_REFRESH_TOKEN`       | no       | Required to expand Spotify playlists since Spotify's 2026 API migration dropped anonymous reads. Generate it with `node scripts/spotify-auth.mjs` (add `http://127.0.0.1:8888/callback` to the app's redirect URIs in the Spotify dashboard; `localhost` is no longer accepted) |
